...
namespace MyDiplomProject
{
    public partial class AddMaterial : Form
    {
        ...
        public AddMaterial(string _user, string _pass)
        {
            User = _user;
            Password = _pass;
            InitializeComponent();
        ...
        private void dataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (!Lock)
            {
                if (items != dataGridView1.Rows.Count)
                {
                    ...
                    cmd.CommandText = "SELECT MATERIAL from SPRAVOCHNIK_MATERIALOV where MATERIAL = '" + str + "'";
                    dr = cmd.ExecuteReader();
                    if (!dr.Read())
                    {
                        cmd.CommandText = "insert into SPRAVOCHNIK_MATERIALOV (MATERIAL) VALUES ('" + str + "')";
                        cmd.ExecuteNonQuery();
                        items = dataGridView1.Rows.Count;
                    }
                    else
                ...
                else
                {
                    ...
                    if (!dr.Read())
                    {
                        ...
                        cmd.CommandText = " UPDATE SPRAVOCHNIK_MATERIALOV set MATERIAL = '" + str + "' where MATERIAL = '" + oldValue + "'";
                        cmd.ExecuteNonQuery();
                    }
                    else
                    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    cmd.CommandText = "SELECT * FROM VESH_DOK, SPRAVOCHNIK_MATERIALOV WHERE VESH_DOK.PK_MATERIAL = SPRAVOCHNIK_MATERIALOV.PK_MATERIAL AND SPRAVOCHNIK_MATERIALOV.MATERIAL = '" + oldValue + "'";
            //    dr = cmd.ExecuteReader();
            //    if (!dr.Read())
            //    {
            //        cmd.CommandText = " DELETE FROM SPRAVOCHNIK_MATERIALOV WHERE MATERIAL = '" + oldValue + "'";
            //        cmd.ExecuteNonQuery();
            ...
            //    }
            //    else
            //        MessageBox.Show("Необходимо удалить или изменить все вещественные доказательства с данным материалом упаковки!", "Удаление данной упаковки невозможно!");
            //}
