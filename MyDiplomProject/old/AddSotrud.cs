...
namespace MyDiplomProject
{
    public partial class AddSotrud : Form
    {
        ...
        public AddSotrud(string _User, string _Pass)
        {
            User = _User;
            Password = _Pass;
            InitializeComponent();
        ...
        void UpDate()
        {
            ...
            //cmd.CommandText = "SELECT YRANITEL.FIO,SPRAVOCHNIK_DOLGNOSTEI.NAZVANIE,SPRAVOCHNIK_NOMEROV.NOMER FROM YRANITEL , SPRAVOCHNIK_DOLGNOSTEI, SPRAVOCHNIK_NOMEROV WHERE SPRAVOCHNIK_DOLGNOSTEI.PK_DOLGNOST = YRANITEL.PK_DOLGNOST AND SPRAVOCHNIK_NOMEROV.PK_NOMER = YRANITEL.PK_NOMER";
        ...
        private void AddSotrud_Load(object sender, EventArgs e)
        {
            UpDate();
        }
        ...
        private void dataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //        cmd.CommandText = "SELECT YRANITEL.FIO,SPRAVOCHNIK_DOLGNOSTEI.NAZVANIE,SPRAVOCHNIK_NOMEROV.NOMER FROM YRANITEL , SPRAVOCHNIK_DOLGNOSTEI, SPRAVOCHNIK_NOMEROV WHERE YRANITEL.FIO = '" + str0 + "' AND YRANITEL.PK_DOLGNOST = (SELECT SPRAVOCHNIK_DOLGNOSTEI.PK_DOLGNOST FROM SPRAVOCHNIK_DOLGNOSTEI WHERE SPRAVOCHNIK_DOLGNOSTEI.NAZVANIE = '" + str1 + "') AND YRANITEL.PK_NOMER = (SELECT SPRAVOCHNIK_NOMEROV.PK_NOMER FROM SPRAVOCHNIK_NOMEROV WHERE SPRAVOCHNIK_NOMEROV.NOMER = '" + str2 + "')";
            //        dr = cmd.ExecuteReader();
            ...
            //            {
            //                cmd.CommandText = "INSERT INTO YRANITEL (FIO,PK_DOLGNOST,PK_NOMER) VALUES ('" + str0 + "', (SELECT PK_DOLGNOST FROM SPRAVOCHNIK_DOLGNOSTEI WHERE SPRAVOCHNIK_DOLGNOSTEI.NAZVANIE = '" + str1 + "'), (SELECT SPRAVOCHNIK_NOMEROV.PK_NOMER FROM SPRAVOCHNIK_NOMEROV WHERE SPRAVOCHNIK_NOMEROV.NOMER = '" + str2 + "'))";
            //                cmd.ExecuteNonQuery();
            //                items = dataGridView1.Rows.Count;
            //            }
            //            else
            //            {
            //                cmd.CommandText = "UPDATE YRANITEL SET FIO = '" + str0 + "', PK_DOLGNOST = (SELECT SPRAVOCHNIK_DOLGNOSTEI.PK_DOLGNOST FROM SPRAVOCHNIK_DOLGNOSTEI WHERE SPRAVOCHNIK_DOLGNOSTEI.NAZVANIE = '" + str1 + "'), PK_NOMER = (SELECT SPRAVOCHNIK_NOMEROV.PK_NOMER FROM SPRAVOCHNIK_NOMEROV WHERE SPRAVOCHNIK_NOMEROV.NOMER = '" + str2 + "') WHERE FIO = '" + oldCell0 + "' AND PK_DOLGNOST = (SELECT SPRAVOCHNIK_DOLGNOSTEI.PK_DOLGNOST FROM SPRAVOCHNIK_DOLGNOSTEI WHERE SPRAVOCHNIK_DOLGNOSTEI.NAZVANIE = '" + oldCell1 + "') AND PK_NOMER = (SELECT SPRAVOCHNIK_NOMEROV.PK_NOMER FROM SPRAVOCHNIK_NOMEROV WHERE SPRAVOCHNIK_NOMEROV.NOMER = '" + oldCell2 + "')";
            //                cmd.ExecuteNonQuery();
            //            }
            //        }
            //        else
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    cmd.CommandText = "SELECT * FROM YRANITEL, ACT WHERE YRANITEL.PK_HRANITEL = ACT.PK_HRANITEL AND YRANITEL.FIO = '" + oldCell0 + "' AND YRANITEL.PK_DOLGNOST = (SELECT SPRAVOCHNIK_DOLGNOSTEI.PK_DOLGNOST FROM SPRAVOCHNIK_DOLGNOSTEI WHERE SPRAVOCHNIK_DOLGNOSTEI.NAZVANIE = '" + oldCell1 + "') AND YRANITEL.PK_NOMER = (SELECT SPRAVOCHNIK_NOMEROV.PK_NOMER FROM SPRAVOCHNIK_NOMEROV WHERE SPRAVOCHNIK_NOMEROV.NOMER = '" + oldCell2 + "')";
            //    dr = cmd.ExecuteReader();
            //    if (!dr.Read())
            //    {
            //        cmd.CommandText = "DELETE FROM YRANITEL WHERE YRANITEL.FIO = '" + oldCell0 + "' AND YRANITEL.PK_DOLGNOST = (SELECT SPRAVOCHNIK_DOLGNOSTEI.PK_DOLGNOST FROM SPRAVOCHNIK_DOLGNOSTEI WHERE SPRAVOCHNIK_DOLGNOSTEI.NAZVANIE = '" + oldCell1 + "') AND YRANITEL.PK_NOMER = (SELECT SPRAVOCHNIK_NOMEROV.PK_NOMER FROM SPRAVOCHNIK_NOMEROV WHERE SPRAVOCHNIK_NOMEROV.NOMER = '" + oldCell2 + "')";
            //        cmd.ExecuteNonQuery();
